 ...
 class theme_essential_core_renderer extends theme_bootstrapbase_core_renderer {
    ...
    */
    ...
    public function login_info($withlinks = null) {
        ...
        if (empty($course->id)) {
            ...
            if (isguestuser()) { //el usuario es Invitado?
                ...
                if ($withlinks) {
                    $loggedinas .= " <a class=\"dropdown-toggle\" data-toggle=\"dropdown\" href=\"#\"><i class='icon icon-user icon-white'></i> Mi perfil <b class='caret'></b></a>".$this->getDropdownPerfil();
                } else {
                    $loggedinas .= $picture;
                }
            }
    ...
    public function getFormLogin()
    {
        global $CFG;
        ob_start();
        require(dirname(__FILE__).'/../login/form.html');
        $res = ob_get_contents();
        ob_get_clean();
